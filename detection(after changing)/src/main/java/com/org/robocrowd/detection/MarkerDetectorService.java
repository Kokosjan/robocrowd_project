...
public class MarkerDetectorService extends ScheduledService<Image> {
	...
	protected Task<Image> createTask(){
		return new Task<Image>() {
			...
			//	Point pt_c = new Point(205,200);
				int radius = 130;
				Imgproc.line(inputImage, start_point, end_point , new Scalar(0,0,255), 2);
				Imgproc.ellipse(inputImage, new Point(end_point.x+130, end_point.y), new Size(radius,radius), 0, 180 , 
						360 , new Scalar(255,0,0), 2, 8, 0);
				Imgproc.line(inputImage, new Point(end_point.x+260, end_point.y), new Point(end_point.x+260, end_point.y+70) , new Scalar(0,0,255), 2);
				Imgproc.line(inputImage, new Point(end_point.x+260, end_point.y+70), start_point , new Scalar(0,255,0), 2);
				//Output distance 
				...
				else if (t_working) turn_right2(mark_centers, rnd_point3, out, angle , side);
				else if (r_working) {if (r_working_out) {out.sendMessage("Stop"); System.out.println("Stop coord" + rnd_point2[0] + " " + rnd_point2[1]); r_working = false;} else {out.sendMessage("round");
				System.out.println("Current coord" + rnd_point2[0] + " " + rnd_point2[1]);
				}
				}
	...
	private void turn_right2 (ArrayList<double[]> mark_centers,double[] rnd_point, TCPConnection out, 
			double angel, String side) {
		if (st_con) {
			
		//	String side = "r";
			angel = Math.toDegrees(angel);
			String angl = Double.toString(angel);
			System.out.println("Sending message to turn " + "right " + angl + " " + side);
			out.sendMessage("right " + angl + " " + side);
			/*st_cen = mark_centers.get(0);
			st_end = angleturn_coordinates(mark_centers.get(0), rnd_point, angel );
			System.out.println("Start coordinates of center are " + st_cen[0] + " " + st_cen[1]);
			System.out.println("Start coordinates of point are " + rnd_point[0] + " " + rnd_point[1]);
		...
		if (out.coms.charAt(0) == 'S') {
				
				//out.getMessage() == "Stop"	) {
		//	out.sendMessage("Stopping");
			System.out.println("Stop");
			out.coms = "Null";
		//	System.out.println("Current coordinates of point " + rnd_point[0] + 
			//		" " + rnd_point[1]);
			...
			//System.exit(1);
			  ...
			  //System.out.println("Client: " + fromUser);
          //  out.sendMessage("right 90");
			System.out.println(out.coms.charAt(0));
