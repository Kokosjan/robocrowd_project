...
public class CC_Controller {
	// FXML buttons
		...
		private Mat intrinsic1;
		private Mat distCoeffs1;
		private boolean isCalibrated;
		public int counterpic;
		...
		{	
			...
			this.intrinsic1 = new Mat(3, 3, CvType.CV_32FC1);
			this.distCoeffs1 = new Mat(1,5, CvType.CV_32FC1);
			this.successes = 0;
				...
				try
				{
					...
					if (!frame.empty())
					{
						...
						if (this.isCalibrated)
						{
							...
							//using created data from python calibration
							...
							double data1[] = new double[]{6.23054572e-02, -2.45866533e-01,  1.79145670e-04,  4.08462722e-03,
									   2.58571738e-01};
							intrinsic1.put( 0, 0, data);
							distCoeffs1.put(0, 0, data1);
							//System.out.println("Camera Matrix:\n" + intrinsic1.dump());
							
							
							//to use javva calib use intrinsic and distCoeffs
							Calib3d.undistort(frame, undistored, intrinsic1, distCoeffs1);
		...
		private void findAndDrawPoints(Mat frame)
		{
			...
			if (this.successes < this.boardsNumber)
			{
				...
				if (found)
				{
					...
					//saving some frames for future calib in python
				//	Imgcodecs.imwrite("C:\\OpenCV_Project\\Input"+counterpic+".jpg", grayImage);
			 //		counterpic++;
					...
					//System.out.println(imageCorners); 
		...
		private void calibrateCamera()
		{
			...
			System.out.println("Distortion coeffs:\n" + distCoeffs.dump());
